...
def draw_top_view(ax, correct=True, title=None, plot_legend=False, far_gcp_x=None, far_gcp_y=None, near_gcp_x=None, near_gcp_y=None):
    """Draw everything the same, except for the GCP locations, which are passed as arguments."""
    print(far_gcp_x)
    if any([far_gcp_x is None, far_gcp_y is None, near_gcp_x is None, near_gcp_y is None]):
        raise ValueError("GCP coordinates must be provided as arguments to draw_top_view()")
    # 1. Land background
    ...
    # 7. plot a red cross or a green check mark around coordinate 4, 8 to indicate whether the GCP placement is correct 
    # or not.
    if correct:
        ax.plot(4, 8, marker="$\checkmark$", markersize=20, markeredgecolor="#33FF33", markerfacecolor="none", markeredgewidth=5)
    else:
        ax.plot(4, 8, marker="X", markersize=20, markeredgecolor="red", markerfacecolor="red", markeredgewidth=4)
...
gcps = [
    {
        "far_gcp_x": far_gcp_x,
        "far_gcp_y": far_gcp_y,
        "near_gcp_x": near_gcp_x,
        "near_gcp_y": near_gcp_y,
    },
    {
        "far_gcp_x": np.array([15.5, 17.0, 16.5, 15.0, 15.3]),
        "far_gcp_y": far_gcp_y + np.array([-0.9, -1.0, 1.4, 2.5, 3.9]),
        "near_gcp_x": np.array([14.8, 15.0, 15.7]),
        "near_gcp_y": near_gcp_y + np.array([-2.5, -1.0, 0.5]),
    },
    {
        "far_gcp_x": far_gcp_x,
        "far_gcp_y": far_gcp_y,
        "near_gcp_x": near_gcp_x + np.array([-5.0, -1.0, 5.0]),
        "near_gcp_y": near_gcp_y + 17.5,
    },
    {
        "far_gcp_x": far_gcp_x + np.array([-3.0, 0.0, 0.0, 1.0, 3.3]),
        "far_gcp_y": far_gcp_y,
        "near_gcp_x": near_gcp_x + np.array([-1.0, 0.0, 1.0]),
        "near_gcp_y": near_gcp_y,
    },
...
titles = [
    f"$\\bf{{a}}$. Good GCP placement",
    f"$\\bf{{b}}$. Bottom-top collinear placement (left-right missing)",
    f"$\\bf{{c}}$. Left-right collinear placement (near-shore missing)",
    f"$\\bf{{d}}$. GCPs too far on the edge (distortion overconstraint)"
]

fig, axs = plt.subplots(figsize=(15, 15), nrows=2, ncols=2)


for n, (gcp, title) in enumerate(zip(gcps, titles)):
    if n == 0:
        plot_legend = True
        correct = True
    else:
        ...
        correct = False
    # print(gcp)
    draw_top_view(ax=axs[n // 2, n % 2], correct=correct, title=title, plot_legend=plot_legend, **gcp)
