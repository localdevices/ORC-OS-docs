...
.. note::
   ...
   and can be estimated by using several calibration targets. At minimum 6 targets are
   needed to estimate the camera pose, but more targets will lead to a more accurate estimation.
   ...
   control information in the calibration targets. 8 points is a good number for this.
...
how to spread and collect control points is given in :ref:`the field survey documentation <field_survey>`.
...
To perform the calibration of the pose, go through the following steps:

1. Go to the "Camera pose" tab.
2. Load the control points from a file or manually enter them into a list in the designated area. You can add
   points using the "+ Add point" button. Make sure you **DO NOT** mix up X and Y coordinates, as this will
   result in a mirrored view on your control points.
3. Once all control points are added, click on the dot left of the first point to start identifying which pixels
   in the video objective correspond to the control points. The point will become solid, and then you can 
...
5. If you make a mistake, no problem, just click on the dot left of the point you wish to modify and click again on the 
   desired pixel on your control points.
...
.. tip::
   ...
   * Use the rainbow color coding or numbering (if you are color blind) to identify if the chosen point corresponds with
     the right coordinates.
 
After you are satisfied, we recommend clicking on the "Save" button to ensure you do not loose any work. The control points
...
1. you have at minimum 6 control points
2. all control points have a X, Y, Z coordinate and a selected row and column

Click on the validate button. After a few seconds, you will see colored "+" signs appear roughly on top of the selected
pixel control points, and you will see a message in the "Camera pose" tab, indicated how large the absolute average error is on 
each control point. Basically, the validation procedure estimates the pose, and then reconstructs the real-world coordinates
from the pixel coordinates and estimates how far apart these are from your field measurements. If this error is larger then
0.1 meter, the message will be an orange colored warning and you may want to change or resurvey. 
Good measurements should result in an error of around 0.05 meters.

.. tips::

   * If your area of interest and distances between control points are very large, an error of 0.1 meters is not necessarily
   ...
   * Leave your control points in place while checking the camera pose and validation. If errors are large, you can still 
     resurvey before taking out the control points.
