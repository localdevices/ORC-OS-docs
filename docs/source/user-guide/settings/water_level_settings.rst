...
-------------------------------
.. screenshot:: http://localhost:5173/water_level
  :browser: chromium
  :viewport-width: 1280
  :viewport-height: 960
  :color-scheme: dark
...
4. When uploading, you must ensure that the device is at that moment capable to run the script. For instance, when you
   are uploading a script that calls an API, you must be connected to the internet. If the script reads a water level
   from a connected sensor, the water level device must be properly connected and communicating with the device.
